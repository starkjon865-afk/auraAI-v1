...
export const Route = createFileRoute("/")({
  component: UserView,
  head: () => ({
    meta: [{ title: "User Workspace · Parity AI" }],
  }),
});
...
function parseMarkdown(text: string) {
  ...
  const parseInline = (str: string) => {
    ...
    return parts.map((part, idx) => {
      if (idx % 2 === 1) {
        return (
          <code key={idx} className="bg-muted/95 px-1.5 py-0.5 rounded text-xs text-primary border border-border/60 font-mono font-semibold">
            {part}
          </code>
        );
      }
      ...
      return boldParts.map((bPart, bIdx) => {
        if (bIdx % 2 === 1) {
          return (
            <strong key={bIdx} className="font-bold text-white tracking-wide font-display">
              {bPart}
            </strong>
          );
        }
        
        // Italics parsing: *italic*
        const italicParts = bPart.split(/\*([^*]+)\*/g);
        return italicParts.map((iPart, iIdx) => {
          if (iIdx % 2 === 1) {
            return (
              <em key={iIdx} className="italic text-neutral-200">
                {iPart}
              </em>
            );
          }
  ...
  lines.forEach((line, lineIdx) => {
    ...
    if (trimmed.startsWith("```")) {
      if (inCodeBlock) {
        // Close code block
        elements.push(
          <div key={`code-${lineIdx}`} className="relative my-4 group rounded-xl border border-border/80 bg-muted/40 overflow-hidden shadow-inner">
            {codeLanguage && (
              <div className="absolute right-3 top-2.5 text-[9px] uppercase tracking-wider text-muted-foreground/80 font-mono font-bold select-none">
                {codeLanguage}
              </div>
            )}
            <pre className="p-4 overflow-x-auto text-xs font-mono text-primary leading-relaxed select-all max-w-full scrollbar-thin">
    ...
    if (!trimmed) {
      if (inList) {
        elements.push(
          <ul key={`list-${lineIdx}`} className="list-disc pl-6 my-4 space-y-2 text-muted-foreground/90">
            {listItems}
          </ul>
        );
        listItems = [];
    ...
    if (trimmed === "---" || trimmed === "***" || trimmed === "___") {
      if (inList) {
        elements.push(<ul key={`list-${lineIdx}`} className="list-disc pl-6 my-4 space-y-2">{listItems}</ul>);
        listItems = [];
        inList = false;
      }
      elements.push(<hr key={lineIdx} className="my-6 border-border/40" />);
      return;
    }
    ...
    if (trimmed.startsWith("# ")) {
      ...
      elements.push(
        <h1 key={lineIdx} className="text-2xl font-bold text-white mt-6 mb-4 border-b border-border pb-2 tracking-tight font-display">
    ...
    // Heading level 2: ## Header
      ...
      elements.push(
        <h2 key={lineIdx} className="text-xl font-bold text-white mt-5 mb-3 tracking-tight font-display">
    ...
    // Heading level 3: ### Header
      ...
      elements.push(
        <h3 key={lineIdx} className="text-lg font-bold text-white mt-4 mb-2 tracking-tight font-display">
    ...
    // Bullet lists: - item or * item
      ...
      inList = true;
      listItems.push(
        <li key={lineIdx} className="leading-relaxed pl-1 text-muted-foreground/90 hover:text-foreground transition-colors duration-150">
          {parseInline(trimmed.slice(2))}
        </li>
      );
    }
    // Numbered lists: 1. item
      ...
      elements.push(
        <div key={lineIdx} className="pl-4 border-l-2 border-primary/40 my-3 py-1 bg-primary/5 rounded-r-lg">
  ...
  if (inList) {
    elements.push(
      <ul key={`list-end`} className="list-disc pl-6 my-4 space-y-2 text-muted-foreground/90">
        {listItems}
      </ul>
    );
  }
...
function UserView() {
  ...
  const INITIAL_GREETING = React.useMemo(() => [
    {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'assistant'; content: string }[]>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("parity_sdg5_user_chat");
      if (saved) {
        try {
  ...
  // Persistent sync effect to securely save updated chat history
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem("parity_sdg5_user_chat", JSON.stringify(messages));
    }
  }, [messages]);
  ...
  const handleClearHistory = () => {
    if (typeof window !== "undefined") {
      localStorage.removeItem("parity_sdg5_user_chat");
    }
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setMessages([
        ...
        {
          role: 'assistant' as const,
          content: `An error occurred: ${errMsg}. Please verify that VITE_API_KEY or API_KEY is set correctly in your Vercel Environment Variables.`
        }
      ]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <RouteGuard allowedRoles={["user", "admin"]} currentRole={userRole}>
      <div className="p-6 lg:p-10 max-w-5xl mx-auto flex flex-col h-[calc(100vh-80px)]">
        <header className="mb-6 shrink-0">
          ...
          <p className="mt-1 text-muted-foreground text-sm max-w-2xl">
            Powered by Parity AI. Work with your mentor to calibrate a week-by-week upskilling path that aggressively closes representation gaps in high-paying tech domains.
          </p>
        </header>
        ...
        <div className="flex-1 min-h-0 border border-border bg-card/60 backdrop-blur-md rounded-2xl relative overflow-hidden shadow-2xl flex flex-col mb-4">
          ...
          <div className="px-6 py-3.5 border-b border-border/40 bg-muted/20 flex items-center justify-between shrink-0 z-10">
            <div className="flex items-center gap-2">
              <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
              <span className="text-xs font-semibold tracking-wide text-muted-foreground/90 uppercase">
                Mentor Session Live
              </span>
            </div>
            <Button
              ...
              variant="ghost"
              size="sm"
              className="h-8 gap-1.5 px-3 rounded-lg text-xs font-medium text-muted-foreground hover:text-rose-400 hover:bg-rose-500/10 active:bg-rose-500/15 border border-transparent hover:border-rose-500/10 transition cursor-pointer"
            >
          ...
          <form onSubmit={handleSend} className="p-4 border-t border-border bg-card/80 backdrop-blur-md flex items-center gap-3 shrink-0">
            <Input
              ...
              placeholder="Ask your Parity AI mentor anything about breaking into tech..."
              disabled={isLoading}
